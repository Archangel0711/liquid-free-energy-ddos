...
class LAMMPSWorker(BaseWorker):
	...
	def start_lammps(self):
		...
		if pair_style is not None:
			# first token is a file path only for table-like styles; plain
			# style names (eam/fs, ufm, snap, ...) have no file to check
			first_token = pair_style.strip().split(" ")[0]
			if not is_number(first_token):
				assert os.path.exists(first_token)
			for i, line in enumerate(lammps_input_script):
				if len(line)>1:
					not_comment = line.strip()[0]!="#"
		...
		if pair_coeff is not None:
			# first token is a coefficient file for snap/eam, but a plain
			# number for analytic styles like ufm (pair_coeff epsilon sigma)
			first_token = pair_coeff.strip().split(" ")[0]
			if not is_number(first_token):
				assert os.path.exists(first_token)
			for i, line in enumerate(lammps_input_script):
				if len(line)>1:
